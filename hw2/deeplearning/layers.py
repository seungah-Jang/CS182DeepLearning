...
def affine_backward(dout, cache):
    ...
    x_flat = x.reshape(N,-1)
    '''
    dout = [[1, 2],   ← 첫 번째 데이터에 대한 기울기
        [3, 4]]   ← 두 번째 데이터에 대한 기울기
    db = [1 + 3, 2 + 4] = [4, 6]
...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_normalized * (x - mean) * -0.5 * (var + eps) ** (-1.5), axis=0)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        ###########################################################################
        mask = (np.random.rand(*x.shape) >= p)  # True/False 형태
        out = x * mask  # 꺼진 애들은 0이 됨
        out /= (1 - p)        
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ...
        ###########################################################################
        dx = dout * mask  # 드롭아웃 안된 부분만 gradient 전달
        dx /= (1 - dropout_param['p'])      
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
    elif mode == 'test':
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    window = x_padded[n, :, vert_start:vert_end, horiz_start:horiz_end]
...
def conv_backward_naive(dout, cache):
    ...
    # db: sum over dout for each filter
    for f in range(F):
        db[f] = np.sum(dout[:, f, :, :])
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    window = x[n, c, vert_start:vert_end, horiz_start:horiz_end]
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dx[n, c, vert_start:vert_end, horiz_start:horiz_end] += mask * dout[n, c, i, j]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # Apply batchnorm backward on reshaped data
