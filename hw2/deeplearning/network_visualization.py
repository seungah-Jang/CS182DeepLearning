...
def compute_saliency_maps(X, y, model):
    ...
    # 정답 클래스 점수만 골라냄
    correct_scores = scores.gather(1, y.view(-1, 1)).squeeze()  # (N,)

    # backward pass: 정답 클래스에 대한 gradient만 계산
    correct_scores.backward(torch.ones_like(correct_scores))

    # saliency map: 입력 이미지에 대한 gradient의 절댓값 최대값
    saliency = X.grad.abs().max(dim=1)[0]  # (N, H, W)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):  
        ...
        with torch.no_grad():
            g = X_fooling.grad
            dX = learning_rate * g / (g.norm() + 1e-8)
            X_fooling += dX
...
def update_class_visulization(model, target_y, l2_reg, learning_rate, img):
    ...
    ########################################################################
    scores = model(img)
    loss = scores[0, target_y] - l2_reg * (img**2).sum()  # L2 정규화 포함
    loss.backward()
