...
#         self.dropout = your_code
    ...
    def forward(self,x):
        ...
        # sequence_length : 단어 개수
        # (1) 단어 인덱스 → 임베딩 벡터
        embeds = self.embedding(x)  # shape: [batch_size, seq_len, rnn_size]
        
        # (2) 임베딩 → LSTM에 넣기
        lstm_out, _ = self.lstm(embeds)  # shape: [batch_size, seq_len, rnn_size]
        
        # (3) LSTM 출력 → 단어 예측 (로짓)
        logits = self.output(lstm_out)  # shape: [batch_size, seq_len, vocab_size]
#         lstm_drop = your_code
#         logits = your_code        
        return logits
