...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        pos_embed_deviced = position_embedding.to(inputs.device)
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, inputs, self_attention_mask=None):
        ...
        output = self.feed_forward(res_attn)  # FFN with its own LayerNorm inside
        return output
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):    
        ...
        encdec_attention = self.cross_attention(
                                (norm_target_selfattn, norm_encoder_outputs),  # query, memory
...
class TransformerDecoder(nn.Module):
    ...
    def get_self_attention_mask(self, batch_size, sequence_length, decoder_mask, mask_future):
        if not mask_future:
            ...
            decoder_mask = decoder_mask.to(future_mask.device)  # 디바이스 맞춰주기
