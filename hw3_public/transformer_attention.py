...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        similarity =  th.matmul(queries, keys.transpose(-1, -2)) / th.sqrt(key_dim) # Compute the similarity according to the QKV formula
...
class MultiHeadProjection(nn.Module):
    ...
    def _combine_heads(self, tensor):
        ...
        tensor = tensor.contiguous().view(batch_size, tensorlen, new_feature_size) # Reshape the Tensor to remove the heads dimension and come back to a Rank-3 tensor
        return tensor
        ##########################################################################################
